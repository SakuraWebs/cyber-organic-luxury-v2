  ...
  return (
    <div className="fixed bottom-8 right-8 z-50">
          ...
          >
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-4">
              {messages.map((msg, index) => (
                <div key={index} className={cn("flex gap-3 max-w-[85%]", msg.role === 'user' ? "ml-auto flex-row-reverse" : "")}>
                  <div className={cn("w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-1", msg.role === 'user' ? "bg-brand-gold/20" : "bg-brand-cyan/20")}>
                    {msg.role === 'user' ? <User className="w-3 h-3 text-brand-gold" /> : <Bot className="w-3 h-3 text-brand-cyan" />}
                  </div>
                  <div className={cn("p-3 rounded-sm text-xs leading-relaxed", msg.role === 'user' ? "bg-brand-gold/10 text-white border border-brand-gold/20" : "bg-white/5 text-gray-300 border border-white/10")}>
                    {msg.text}
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 border-t border-white/5 bg-white/5">
              <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-2">
                <input
                  ...
                  className="flex-grow bg-brand-dark border border-white/10 rounded-sm px-4 py-2 text-xs text-white focus:outline-none focus:border-brand-cyan"
                />
                <button type="submit" disabled={isLoading || !input.trim()} className="w-10 h-10 rounded-sm bg-brand-cyan flex items-center justify-center text-brand-dark hover:bg-brand-gold disabled:opacity-50">
      ...
      <motion.button
        ...
        className={cn("w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all duration-500", isOpen ? "bg-white text-brand-dark rotate-90" : "bg-brand-cyan text-brand-dark")}
      >
